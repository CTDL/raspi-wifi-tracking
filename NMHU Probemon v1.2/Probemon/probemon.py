...
def wereOpen():
        ...
        if stopReading == True:
                stopReading = False
                #check files for unsent files and handle them
                checkFiles()
...
def sendEmail(incFile):
        ...
        try:
                ...
                if incFile == "": #if incoming file is a blank string, then perform standard send
                        ...
                        os.system(osString)
                elif incFile != "": #if incoming file has a string, then it's a resend of an unsent file
                        msg = MIMEMultipart()
                        msg['From'] = fromEmail
                        msg['To'] = toEmail
                        msg['Subject'] = ( 'Probemon unsent file resend attempt' )
                        body = 'Probemon detected an unsent file in its log directory. This email is an automated resend of that file.'
                        content = MIMEText(body,'plain')
                        msg.attach(content)

                        #since an existing filename will have the .txt in it, remove that first
                        incFileArr = incFile.split(".")         #seperate NS_XXXX.txt to NS_XXXX and txt
                        file = incFileArr[0]                    #get the NS_XXXX filename only
                        fileNameArr = file.split("_")           #seperate NS_XXXX into NS and XXXX
                        file = fileNameArr[1]                   #get the XXXX filename only
                        
                        oldNameWithPath = "/home/pi/Probemon/logs/NS_"+file+".txt"
                        newNameWithPath = "/home/pi/Probemon/logs/"+file+".csv"                 #rename .txt file to .csv
                        osString = "sudo mv %s %s"  % (oldNameWithPath, newNameWithPath)        #rename old file to new file name
                        fileName = file+".csv"
                        os.system(osString)
                ...
                print("could not connect to server. File not sent!")
        except socket.error:
                print("error creating socket. File not sent!")
...
#function checks all files in logs directory, looking for unsent files- that is, files that begin with NS_
#if files are found, function sends the unsent ones to the specified email address with a specialized note.
#function runs at both startup (right after this function declaration) and also at opening time.
def checkFiles():
        
        print("checking files in logs directory...")
        directory = "/home/pi/Probemon/logs/"
        for filename in os.listdir(directory):
                if filename.startswith("NS_"):
                        print("unsent file found!: %s" %filename )
                        print("Attempting to re-send file...")
                        sendEmail(filename)
        print("filecheck complete.")

                        
#run function at boot
checkFiles()
